...
boost::shared_ptr<Common::Interserver_connection> Foyer_connector::get_client(const Poseidon::Uuid &connection_uuid){
	POSEIDON_PROFILE_ME;

	const Poseidon::Mutex::Unique_lock lock(g_mutex);
	const AUTO(connector, g_weak_connector.lock());
	if(!connector){
		CIRCE_LOG_WARNING("Foyer_connector has not been initialized.");
		return VAL_INIT;
	}
	return connector->get_client(connection_uuid);
}
std::size_t Foyer_connector::get_all_clients(boost::container::vector<boost::shared_ptr<Common::Interserver_connection> > &clients_ret){
	POSEIDON_PROFILE_ME;
...
std::size_t Foyer_connector::safe_broadcast_notification(const Poseidon::Cbpp::Message_base &msg) NOEXCEPT {
	POSEIDON_PROFILE_ME;
...
std::size_t Foyer_connector::clear(long err_code, const char *err_msg) NOEXCEPT {
	POSEIDON_PROFILE_ME;
