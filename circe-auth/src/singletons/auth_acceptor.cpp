...
MODULE_RAII_PRIORITY(handles, INIT_PRIORITY_LOW){
	...
	LOG_CIRCE_INFO("Initializing Auth_acceptor...");
...
boost::shared_ptr<Common::Interserver_connection> Auth_acceptor::get_session(const Poseidon::Uuid &connection_uuid){
	PROFILE_ME;

	const Poseidon::Mutex::Unique_lock lock(g_mutex);
	const AUTO(acceptor, g_weak_acceptor.lock());
	if(!acceptor){
		LOG_CIRCE_WARNING("Auth_acceptor has not been initialized.");
		return VAL_INIT;
	}
	return acceptor->get_session(connection_uuid);
}
std::size_t Auth_acceptor::get_all_sessions(boost::container::vector<boost::shared_ptr<Common::Interserver_connection> > &sessions_ret){
	PROFILE_ME;
...
std::size_t Auth_acceptor::safe_broadcast_notification(const Poseidon::Cbpp::Message_base &msg) NOEXCEPT {
	PROFILE_ME;
...
std::size_t Auth_acceptor::clear(long err_code, const char *err_msg) NOEXCEPT {
	PROFILE_ME;
