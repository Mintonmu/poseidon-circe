...
void Interserver_servlet_container::insert_servlet(boost::uint16_t message_id, const boost::shared_ptr<const Interserver_servlet_callback> &servlet){
	PROFILE_ME;
	DEBUG_THROW_UNLESS(Interserver_connection::is_message_id_valid(message_id), Poseidon::Exception, Poseidon::Rcnts::view("message_id out of range"));
	DEBUG_THROW_UNLESS(servlet, Poseidon::Exception, Poseidon::Rcnts::view("Null servlet pointer"));
	...
	DEBUG_THROW_UNLESS(m_servlets.emplace(message_id, servlet).second, Poseidon::Exception, Poseidon::Rcnts::view("Another servlet for this message_id already exists"));
}
