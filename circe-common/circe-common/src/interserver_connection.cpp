...
public:
	void reset(const boost::array<unsigned char, 32> &seed){
		POSEIDON_THROW_ASSERT(::AES_set_encrypt_key(seed.data(), 256, &m_key) == 0);
		m_chunk.fill(42);
		m_count = 16;
	}
...
public:
	...
	Poseidon::Stream_buffer encode(Poseidon::Stream_buffer magic_payload){
		...
		POSEIDON_THROW_ASSERT(temp.unput() == 0xFF);
		POSEIDON_THROW_ASSERT(temp.unput() == 0xFF);
		POSEIDON_THROW_ASSERT(temp.unput() == 0x00);
		POSEIDON_THROW_ASSERT(temp.unput() == 0x00);
...
protected:
	...
	void perform() FINAL {
		...
		try {
			CIRCE_LOG_TRACE("Dispatching message: message_id = ", m_message_id);
			...
			try {
				...
				CIRCE_LOG_ERROR("Protocol::Exception thrown: mesasge_id = ", m_message_id, ", err_code = ", e.get_status_code(), ", err_msg = ", e.what());
				resp = Interserver_response(e.get_status_code(), e.what());
			} catch(std::exception &e){
				CIRCE_LOG_ERROR("std::exception thrown: mesasge_id = ", m_message_id, ", err_msg = ", e.what());
			...
			CIRCE_LOG_TRACE("Done dispatching message: message_id = ", m_message_id);
		} catch(std::exception &e){
...
{
	CIRCE_LOG_INFO("Interserver_connection constructor: this = ", (void *)this);
}
Interserver_connection::~Interserver_connection(){
	CIRCE_LOG_INFO("Interserver_connection destructor: this = ", (void *)this);
}
...
void Interserver_connection::server_accept_hello(const Poseidon::Uuid &connection_uuid, boost::uint64_t timestamp, Poseidon::Option_map options_resp){
	POSEIDON_PROFILE_ME;
	CIRCE_LOG_INFO("Accepted client HELLO from ", get_remote_info(), " with options ", options_resp);

	const Poseidon::Recursive_mutex::Unique_lock lock(m_mutex);
	POSEIDON_THROW_UNLESS(!m_connection_uuid, Poseidon::Exception, Poseidon::Rcnts::view("server_accept_hello() shall be called exactly once by the server and must not be called by the client"));
...
void Interserver_connection::client_accept_hello(Poseidon::Option_map options_resp){
	POSEIDON_PROFILE_ME;
	CIRCE_LOG_INFO("Accepted server HELLO from ", get_remote_info(), " with options ", options_resp);

	const Poseidon::Recursive_mutex::Unique_lock lock(m_mutex);
	POSEIDON_THROW_UNLESS(m_connection_uuid, Poseidon::Exception, Poseidon::Rcnts::view("client_accept_hello() shall be called exactly once by the client and must not be called by the server"));
	m_options         = STD_MOVE(options_resp);
	layer7_post_set_connection_uuid();
}
void Interserver_connection::send_response(boost::uint64_t serial, Interserver_response resp){
	...
	POSEIDON_THROW_UNLESS((message_id == 0) || is_message_id_valid(resp.get_message_id()), Poseidon::Exception, Poseidon::Rcnts::view("message_id out of range"));
	...
	// Send it.
	CIRCE_LOG_TRACE("Sending user-defined response: remote = ", get_remote_info(), ", hdr = ", hdr, ", message_id = ", resp.get_message_id(), ", payload_size = ", magic_payload.size());
...
try {
	...
	CIRCE_LOG_TRACE("Inflate and dispatch: connection = ", (void *)this);
	...
	CIRCE_LOG_TRACE("Inflate result: ", size_deflated, " / ", size_original, " (", std::fixed, std::setprecision(3), (size_original != 0) ? size_deflated * 100.0l / size_original : 0.0l, "%)");
	...
	if(Poseidon::has_any_flags_of(magic_number, magic_flag_is_response)){
		...
		CIRCE_LOG_TRACE("Received user-defined response: remote = ", get_remote_info(), ", hdr = ", hdr, ", message_id = ", message_id, ", payload_size = ", magic_payload.size());
		// Satisfy the promise.
		...
		hdr.deserialize(magic_payload);
		CIRCE_LOG_TRACE("Received user-defined request: remote = ", get_remote_info(), ", hdr = ", hdr, ", message_id = ", message_id, ", payload_size = ", magic_payload.size());
		...
		CIRCE_LOG_TRACE("Received user-defined notification: remote = ", get_remote_info(), ", message_id = ", message_id, ", payload_size = ", magic_payload.size());
	...
	CIRCE_LOG_ERROR("Protocol::Exception thrown: status_code = ", e.get_status_code(), ", what = ", e.what());
...
try {
	...
	CIRCE_LOG_TRACE("Deflate and send: connection = ", (void *)this);
	...
	const std::size_t size_deflated = encoded_payload.size();
	CIRCE_LOG_TRACE("Deflate result: ", size_deflated, " / ", size_original, " (", std::fixed, std::setprecision(3), (size_original != 0) ? size_deflated * 100.0l / size_original : 0.0l, "%)");
...
void Interserver_connection::layer5_on_receive_control(long status_code, Poseidon::Stream_buffer param){
	POSEIDON_PROFILE_ME;
	POSEIDON_THROW_ASSERT(has_authenticated());

	switch(status_code){
	case Poseidon::Cbpp::status_shutdown:
		CIRCE_LOG_INFO("Received SHUTDOWN frame from ", get_remote_info());
		layer5_shutdown(Poseidon::Cbpp::status_shutdown, static_cast<const char *>(param.squash()));
		break;
	case Poseidon::Cbpp::status_ping:
		CIRCE_LOG_TRACE("Received PING frame from ", get_remote_info(), ": param = ", param);
		layer5_send_control(Poseidon::Cbpp::status_pong, STD_MOVE(param));
		break;
	case Poseidon::Cbpp::status_pong:
		CIRCE_LOG_TRACE("Received PONG frame from ", get_remote_info(), ": param = ", param);
		break;
	default:
		CIRCE_LOG_WARNING("Received CBPP error from ", get_remote_info(), ": status_code = ", status_code, ", param = ", param);
		break;
	}
}
void Interserver_connection::layer4_on_close(){
	...
	{
		const Poseidon::Recursive_mutex::Unique_lock lock(m_mutex);
		POSEIDON_THROW_ASSERT(has_been_shutdown());
...
void Interserver_connection::layer7_client_say_hello(Poseidon::Option_map options_req){
	...
	CIRCE_LOG_INFO("Sending client HELLO to ", get_remote_info(), " with options ", options_req);

	const Poseidon::Recursive_mutex::Unique_lock lock(m_mutex);
	POSEIDON_THROW_UNLESS(!m_connection_uuid, Poseidon::Exception, Poseidon::Rcnts::view("layer7_client_say_hello() shall be called exactly once by the client and must not be called by the server"));
...
boost::shared_ptr<const Promised_response> Interserver_connection::send_request(const Poseidon::Cbpp::Message_base &msg){
	...
	CIRCE_LOG_TRACE("Sending user-defined request: remote = ", get_remote_info(), ", msg = ", msg);
	const boost::uint64_t message_id = msg.get_id();
	POSEIDON_THROW_UNLESS(is_message_id_valid(message_id), Poseidon::Exception, Poseidon::Rcnts::view("message_id out of range"));
...
void Interserver_connection::send_notification(const Poseidon::Cbpp::Message_base &msg){
	...
	CIRCE_LOG_TRACE("Sending user-defined notification: remote = ", get_remote_info(), ", msg = ", msg);
	const boost::uint64_t message_id = msg.get_id();
	POSEIDON_THROW_UNLESS(is_message_id_valid(message_id), Poseidon::Exception, Poseidon::Rcnts::view("message_id out of range"));
...
void wait_for_response(Poseidon::Cbpp::Message_base &msg, const boost::shared_ptr<const Promised_response> &promise){
	...
	CIRCE_LOG_TRACE("Response: ", resp);

	CIRCE_PROTOCOL_THROW_UNLESS(resp.get_err_code() == 0, resp.get_err_code(), Poseidon::Rcnts(resp.get_err_msg()));
	POSEIDON_THROW_UNLESS(resp.get_message_id() != 0, Poseidon::Exception, Poseidon::Rcnts::view("No message payload but status code returned"));
	POSEIDON_THROW_UNLESS(resp.get_message_id() == msg.get_id(), Poseidon::Exception, Poseidon::Rcnts::view("Unexpected response message ID"));
	msg.deserialize(resp.get_payload());
	if(!resp.get_payload().empty()){
		CIRCE_LOG_WARNING("Ignoring ", resp.get_payload().size(), " redundant byte(s) after message ", msg);
	}
}
