...
namespace {
	...
	MODULE_RAII_PRIORITY(, 1000){
		LOG_CIRCE_INFO("Initialize global cipher...");
		...
		for(AUTO(it, users_v.begin()); it != users_v.end(); ++it){
			...
			LOG_CIRCE_TRACE("> Authorized user: ", str);
			const AUTO(pos, str.find(':'));
			if(pos == str.npos){
				LOG_CIRCE_FATAL("Invalid encryption_authorized_user: ", str,  "(Hint: encryption_authorized_user = USERNAME:PASSWORD)");
				DEBUG_THROW(Poseidon::Exception, Poseidon::sslit("Invalid encryption_authorized_user"));
			}
			if(pos == 0){
				LOG_CIRCE_FATAL("Username must not be empty: ", str);
				DEBUG_THROW(Poseidon::Exception, Poseidon::sslit("Username must not be empty"));
			}
			if(pos > 16){
				LOG_CIRCE_FATAL("Username must contain no more than 16 bytes: ", str);
			...
			if(!result.second){
				LOG_CIRCE_FATAL("Duplicate username: ", str);
